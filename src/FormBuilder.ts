...
class FormBuilder {
  ...
  addTextField(name: string, label: string, defaultValue: string = ''): FormBuilder {
    this.fields.push({
      type: 'text',
      name,
      label,
  ...
  addSelectField(name: string, label: string, options: SelectOption[]): FormBuilder {
    this.fields.push({
      type: 'select',
      name,
      label,
  ...
  addCheckbox(name: string, label: string, defaultChecked: boolean = false): FormBuilder {
    this.fields.push({
      type: 'checkbox',
      name,
      label,
  ...
  build(): React.ComponentType {
    ...
    return function FormComponent() {
      ...
      const initialState: FormData = fields.reduce((acc: FormData, field: FormField) => {
      ...
      return (
        <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
          <div id={formId} onSubmit={handleSubmit}>
